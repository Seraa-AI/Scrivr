...
- b356735: PDF export paints the face the layout measured
...
- d1fa767: Exported PDFs now describe themselves, and carry bookmarks.
...
- b356735: Keep apostrophes and quotes in the exported PDF's text layer.
...
- b356735: **Breaking: embedding a face in a PDF now requires the resource to say so.**
...
- b356735: Embed a subset of each face, built with fontkit v2.
...
- b356735: **Breaking: a WOFF or WOFF2 face is refused instead of written into the PDF.**
...
- ace9a88: An extension can style its own mark in a PDF.
  ...
  **Breaking: `PdfSpanStyle` changes shape.** Colours are CSS strings rather than
  pdf-lib triples, and it gains `defaultColor` for a colour a mark supplies by
  being what it is, which loses to an authored `color`. Although the old lane was
  inert, consumer code may already use its published types and needs migration.
  See `docs/export-extensibility.md` for the type and context changes. This is a
...
- b356735: A PDF reproduces the document on screen
...
- b356735: Give ligatures their real width in the exported PDF.
...
- b356735: A weight or slant nobody owns is drawn, not dropped
