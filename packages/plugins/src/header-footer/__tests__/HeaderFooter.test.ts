...
describe("HeaderFooter extension", () => {
  ...
  describe("commands", () => {
    ...
    it("updateHeaderFooter merges partial into existing policy", () => {
      ...
      editor._applyTransaction(
        editor.getState().tr.setDocAttribute("headerFooter", initial),
      );
      ...
      editor._applyTransaction(
        editor.getState().tr.setDocAttribute("headerFooter", updated),
      );
    ...
    it("removeHeaderFooter sets policy to null", () => {
      ...
      editor._applyTransaction(
        editor.getState().tr.setDocAttribute("headerFooter", null),
      );
      expect(editor.getState().doc.attrs["headerFooter"]).toBeNull();
    });
  });
  ...
  //
  // The chromeClick handler is installed via onEditorReady, which only fires
  // on view editors (ones with `surfaces` + `layout`). ServerEditor doesn't
  // satisfy that, so we splice in the minimum surfaces + event bus that the
  // handler needs, then drive it through editor.emit("chromeClick", ...).
  describe("margin double-click bootstraps policy", () => {
