...
<Callout type="info">
  **You do not need to use `ViewManager` directly.** The `<Inscribe />` React component handles
...
Always call `destroy()` before discarding a `ViewManager` instance. The `<Inscribe />` component
calls this in its `useEffect` cleanup.

---

## `<Inscribe />` as a Thin Wrapper

The full implementation of `<Inscribe />` is:
