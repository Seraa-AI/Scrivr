...
description: React bindings for Inscribe — useInscribeEditor hook, Inscribe component, and useInscribeState selector hook.
---
...
`useInscribeState` subscribes to a specific slice of editor state. The component only re-renders when
...
## InscribeContext and useInscribe

<Callout type="info">
  Coming soon. `InscribeContext` and `useInscribe` are not yet available.
</Callout>

In larger applications passing the `editor` instance down as a prop to every toolbar, menu, and
sidebar component gets tedious. `InscribeContext` will let you provide the editor once at the
top of your tree, and `useInscribe` will let any child component consume it without props.

```tsx title="Future API (not yet available)"
import { InscribeContext, useInscribe } from '@inscribe/react';
...
export function EditorApp() {
  ...
  return (
    <InscribeContext.Provider value={editor}>
      <Toolbar />
