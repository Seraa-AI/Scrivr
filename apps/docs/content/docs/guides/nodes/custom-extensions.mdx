...
A node that only *contains* other blocks — a grouping or provenance wrapper — paints nothing of
its own, so it has no strategy to register. Mark its spec `layoutContainer: true` instead and the
layout walker treats it as transparent, laying out its children as independent blocks:
...
Without either a strategy or this flag, a `block+` node reaches the text-block path and lays out
as a single empty line — its children never paint.
