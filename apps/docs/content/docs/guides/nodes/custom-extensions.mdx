...
Extensions need ProseMirror types and helpers — `Node`, `Slice`, `Plugin`, `toggleMark`. Import
them from **`@scrivr/core/pm`**, not from the `prosemirror-*` packages directly:

```ts
import { Plugin, PluginKey, Fragment, toggleMark } from '@scrivr/core/pm';
import type { Node, EditorState } from '@scrivr/core/pm';
```

`@scrivr/core` owns the ProseMirror versions and re-exports them, so your extension runs against
the same instance as the engine. That matters: `instanceof` checks on `Node`, `Slice` and
`Selection` are load-bearing, and a second copy of `prosemirror-model` in the same process breaks
them in ways that surface as unexplained failed transactions. You also don't declare the
dependency yourself.

<Callout type="warn">
  `prosemirror-view` is not re-exported and is not a dependency. Scrivr paints to canvas and has
  no `EditorView`, so `Plugin.spec.view()` and plugin `props` (`handleKeyDown`, `transformPasted`,
  `decorations`) never run. Use `appendTransaction`, `addKeymap`, or `PasteTransformer` instead.
</Callout>
...
### Declaring layout participation

How a node participates in layout is a separate dimension from what it means in the document
tree, so the node spec declares it:

| `layout` | Meaning |
|---|---|
| `{ kind: 'block' }` | The node occupies its own box in the flow, painted by the `BlockStrategy` it registers. This is the default when `layout` is omitted. |
| `{ kind: 'transparent' }` | The node contributes no box of its own. It stays in the editor tree, and its children lay out into the enclosing flow as if the boundary weren't there. |

A node can matter enormously to the model and have no visual existence of its own — it carries
identity, provenance, actions and a lifecycle, but the reader only ever sees its children. That
node is `transparent`:
...
The document keeps its structure — `sourcedBlock → heading, paragraph, paragraph` — while the
layout stream sees `heading, paragraph, paragraph`. Detaching such a node removes the boundary
without touching the content inside it.

Declare `transparent` rather than leaving `layout` off: an undeclared `block+` node falls through
to the text-block path and lays out as a single empty line, painting nothing.
...
import { toggleMark } from '@scrivr/core/pm';
